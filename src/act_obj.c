...
void do_drop (CHAR_DATA * ch, char *argument)
{
    ...
    {
        /* 'drop NNNN coins' */
        int amount, gold = 0, silver = 0, platinum = 0;
...
void wear_obj (CHAR_DATA * ch, OBJ_DATA * obj, bool fReplace)
{
    ...
    if (CAN_WEAR (obj, ITEM_WEAR_NOSE))
    {
        if (!remove_obj (ch, WEAR_NOSE, fReplace))
            return;
        act ("$n wears $p in $s nose.", ch, obj, NULL, TO_ROOM);
        act ("You wear $p in your nose.", ch, obj, NULL, TO_CHAR);
    ...
    if (CAN_WEAR (obj, ITEM_WEAR_EAR))
    {
        if (get_eq_char (ch, WEAR_EAR_1) != NULL
            && get_eq_char (ch, WEAR_EAR_2) != NULL
            && !remove_obj (ch, WEAR_EAR_1, fReplace)
            && !remove_obj (ch, WEAR_EAR_2, fReplace))
            return;
    ...
    if (CAN_WEAR (obj, ITEM_WEAR_SHIELD))
    {
        ...
        levitate = ( ( get_skill ( ch, gsn_shield_levitation ) != 0 ) &&
		      ...
		      || skill_table[gsn_shield_levitation].
                       skill_level[ch->class2] <= ch->level2  
		      || skill_table[gsn_shield_levitation].
                       skill_level[ch->class3] <= ch->level3 ) );
        if (weapon != NULL && ch->size < SIZE_LARGE
            && IS_WEAPON_STAT (weapon, WEAPON_TWO_HANDS) && !levitate)
        {
...
//        send_to_char("Mota appreciates your offer and may accept it later.\n\r", ch);
    ...
    if (obj->item_type == ITEM_CORPSE_PC)
    {
        if (obj->contains)
        {
        sprintf (buf, "%s wouldnt like that.\n\r",god_table[ch->god].name);
...
//act( "$g gives you one silver coin for your sacrifice.",ch, NULL, NULL, TO_CHAR );
        send_to_char (buf, ch);
...
//    act ("$n sacrifices $p to Mota.", ch, obj, NULL, TO_ROOM);
	...
	reward = number_range(1,100);



	if (reward == 1)
	{
        sprintf(buf,"%s gives you 5 quest points.\n\r", god_table[god].name);
        send_to_char (buf, ch);
	ch->questpoints += 5;
	}
        if (reward == 2)
        {
        sprintf(buf,"%s gives you 500 silver coins.\n\r", god_table[god].name);
        send_to_char (buf, ch);
        ch->silver += 500;
        }
        if (reward == 3)
        {
        sprintf(buf,"%s gives you 500 gold coins.\n\r", god_table[god].name);
        send_to_char (buf, ch);
        ch->gold += 500;
        }
        if (reward == 4)
        {
        sprintf(buf,"%s gives a restore.\n\r", god_table[god].name);
        send_to_char (buf, ch);
	ch->hit = ch->max_hit;
        ch->mana = ch->max_mana;
        ch->move = ch->max_move;
        ...
        if (reward == 5)
        {
        sprintf(buf,"%s gives you 5 practice points.\n\r", god_table[god].name);
        send_to_char (buf, ch);
        ch->practice += 5;
        }
        if (reward == 6)
        {
        sprintf(buf,"%s gives you 5 trains.\n\r", god_table[god].name);
        send_to_char (buf, ch);
        ch->train += 5;
        }
...
//    char buf[MAX_STRING_LENGTH]; /* overkill, but what the heck */
    ...
    if (obj == NULL)
    {
        send_to_char ("You have no such thing in your bag.\n\r", ch);
        return;
    }
...
/*    if ( (get_eq_char (ch,WEAR_SHIELD) != NULL) ||
    ...
    {
        send_to_char ("You cannot use a secondary weapon while using a shield or holding and item\n\r", ch);
        return;
    }
*/
...
/* char CAN use the item! that didn't take long at all */
