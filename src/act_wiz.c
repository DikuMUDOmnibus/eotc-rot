...
void do_guild( CHAR_DATA *ch, char *argument )
{
	...
	{
	sprintf(buf,"%s must be at least level %d to join %s clan.\n\r",
...
void do_mset (CHAR_DATA * ch, char *argument)
{
    ...
    if (arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0')
    {
        ...
        send_to_char ("    security hours trust quest\n\r", ch);
        return;
    }
    ...
    {
    sprintf( buf, "You give them %d quest points\n\r",value);
    send_to_char(buf,ch);
    ...
    {
        if (value > 215 || value < 0)
        {
            send_to_char("Trust range is 1 to 215.\n\r", ch);
            return;
        }
...
//     send_to_char(buf,ch);	
    ...
    for (d = descriptor_list; d; d = d_next)
    {
        ...
        else
        {
            ...
            {
                write_to_descriptor (d->descriptor,
                                     "Since you are level one, and level one characters do not save, you gain a free 
level!\n\r",
                                     0);
                advance_level (och);
