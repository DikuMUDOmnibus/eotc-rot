...
{ "Petition",   "Pfile weirdness and oddities", 0,      1,      "imm", DEF_NORMAL, 90, NULL, FALSE },
{ "RP",         "Role-playing notes",           0,      1,      "all", DEF_NORMAL, 90, NULL, FALSE },
{ "Council",    "Immortal Discussion",          L_IMM,  L_IMM,  "imm", DEF_NORMAL, 1825, NULL, FALSE },
{ "Punishments","Punishment Announcements",     L_IMM,  L_IMM,  "imm", DEF_NORMAL, 1825, NULL, FALSE },
{ "Gods",       "Gods Discussion",              L_SUP,  L_SUP,  "imm", DEF_NORMAL, 1825, NULL, FALSE }
};
...
void do_board (CHAR_DATA *ch, char *argument)
{
	...
	if (!argument[0]) /* show boards */
	{
		...
		count = 1;
		send_to_char ("{YNum          Name Unread Read/Write Description{x\n\r"
		              "{R==== ============ ====== ========== ============================={x\n\r",ch);
		for (i = 0; i < MAX_BOARD; i++)
		{
			...
			{ 
				printf_to_char (ch, "{y({Y%2d{y) {8%12s{x {8[{Y%s%4d{8]    {8[{Y%s{8]   %s{x\n\r", 
				                   count, boards[i].short_name, unread ? "{Y" : "{Y", 
				                    unread, 			              
