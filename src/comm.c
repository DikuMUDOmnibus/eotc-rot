           ...
           if ( ( victim = ch->fighting ) != NULL && can_see ( ch, victim ) )
            {
                ...
                if ( percent == 100 )
                    ...
                    sprintf ( meter, "[            {x]" );
                sprintf ( buf, "%s{x: %s {B[{R%c%.2f{B]{x\n\r",
                          IS_NPC ( victim ) ? victim->short_descr : victim->
                          name, meter, '%', percent );
                buf[0] = UPPER ( buf[0] );
                send_to_char ( buf, ch );
           ...
           char buf[MSL];  //more than plenty
            ...
            ch = d->original ? d->original : d->character;
			    ...
			    if (IS_SET (ch->comm, COMM_BRIEF3))
                            ...
                            {
                            sprintf(buf,
                                    "{8You've dealt {Y%d {8attacks causing {Y%d {8damage. {R({Y%d {8average{R){x\n\r",
                            ...
                            {
                            sprintf(buf,
                                    "{8You've been struck by {Y%d {8attacks, causing {R%d {8damage. {R({Y%d {8average{R){x\n\r",
