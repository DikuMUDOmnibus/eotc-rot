...
{
    return 17 + (ch->played + (int) (current_time - ch->logon)) / 3600;
}
...
char *affect2_bit_name( int vector )
{
    ...
    if (vector & AFF2_WEBBED         ) strcat (buf, " webbed");
...
char *act_bit_name (int act_flags)
{
    ...
    if (IS_SET (act_flags, ACT_IS_NPC))
    {
        ...
        if (act_flags & ACT_WARRIOR)
            strcat (buf, " warrior");
        if (act_flags & ACT_PALADIN)
            strcat (buf, " paladin");
        if (act_flags & ACT_ANTIPALADIN)
            strcat (buf, " antipaladin");
        if (act_flags & ACT_NECROMANCER)
            strcat (buf, " necromancer");
        if (act_flags & ACT_RANGER)
            strcat (buf, " ranger");
        if (act_flags & ACT_MONK)
            strcat (buf, " monk");
